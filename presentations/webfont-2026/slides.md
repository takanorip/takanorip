...
<p>先に「なぜ選ぶのか」を短く置き、そのあと重い・ずれるを止め、最後に選び方と新しい道具へ進みます。</p>

<ol class="agenda-list">
<li><span class="n">01</span>なぜウェブフォントか</li>
<li><span class="n">02</span>配信を軽くする</li>
<li><span class="n">03</span>表示をずらさない</li>
<li><span class="n">04</span>書体の選び方</li>
<li><span class="n">05</span>2026年の道具</li>
</ol>
...
<p class="muted">印刷では当たり前だった書体指定を、画面に戻す話です。</p>
...
<p>トーン、階層、読みやすさは、書体で大きく変わる。Webは画面の大半が文字なので、色や余白より影響が大きいことすらある。見た目のためだけではない。読めること、迷わないこと、信頼できることも、書体の仕事です。</p>
...
# だから、選べることには<span class="marker">価値がある</span>。

<p class="muted">重い・ずれるの対策は必要です。それでも、選べない状態に戻す理由にはならない。ここから、その対策に入ります。</p>
...
<p class="muted">包んで圧縮し、使わない字は送らない。切り方はサービスごとに違う。</p>
...
<p>包めるようになっても、日本語はそのままだと重い。ラテン1言語なら数百グリフで足りることが多い。日本語は漢字があり、同じ圧縮をしても桁が違う。</p>
...
<p class="note">新規で TTF や WOFF 1 を配る理由は、ほぼない。古いブラウザ向けに WOFF を残すなら、WOFF2 を先に書く。圧縮だけでは、日本語の全部入りはまだ大きい。</p>

---

# 使わない文字は、送らない

<p>次の手は、切ること。<code>unicode-range</code> を分けると、ページに出ていない文字のファイルは取りにいかない。ラテンと日本語を別ファイルにするだけでも、英字だけの画面は軽くなる。</p>
...
<p class="note">CJK の配信サービスは、この「切る」を極端に細かくする。切り方は、大きく2つある。</p>

---

# 切り方は、大きく2つある

<p>同じ「使わない字は送らない」でも、いつ切るかが違う。2012年の TypeSquare はページ表示時。2018年の Google Fonts 日本語は、事前のスライス。</p>

<div class="cols">
<div class="card">
<h3>都度つくる</h3>
<p>ページの文字を読んで、そのグリフだけのファイルをサーバーが作る。TypeSquare のダイナミックサブセット。</p>
</div>
<div class="card">
<h3>事前に切る</h3>
<p>頻度やコードポイントでスライスを置いておき、ブラウザが <code>unicode-range</code> で取る。Google Fonts の CJK。</p>
</div>
</div>
...
<p>Google Fonts 側では、ページの文字を見て、当たった <code>unicode-range</code> のファイルだけ取りにいく。切り方を細かくできるのは、小さいファイルを同時に取れるようになったからだ。</p>
...
<p>キャッシュと余剰のトレードオフになる。どちらが正しいではなく、ページの変わり方で選ぶ。</p>
...
<p>どこからファイルを取るかで、許諾、キャッシュ、制御の細かさが変わる。見た目が同じでも、運用の責任は違う。許諾の中身は、04で戻ります。</p>
...
<p class="muted">軽くしたあとも、待ち方と寸法を決めないと、ページは跳ねる。</p>
...
| `optional` | 短い待ちのあと、間に合わなければ使わない | 安定優先。再訪でキャッシュが効く |
| `fallback` | `optional` より待ちが長く、そのあと swap | 見出しと本文のあいだ |
| `block` | 隠して待つ。FOIT が長くなる | ロゴ1語など、ほぼ使わない |
...
<p class="muted">圧縮、分割、待ち方、寸法。どれも「書体をやめる」以外の手段です。次は、何を選ぶかの話です。</p>
...
<p class="muted">系統と本数、UDの意味、使ってよい経路。この順で決める。</p>
...
<p>画面に出ているから使ってよい、にはならない。配る行為と、表示する行為は、契約上は別物です。02の「どこから取るか」は、ここで契約になる。</p>
...
<p class="muted">静的スライスの先と、CSS から触れること。全部を今日入れなくていい。</p>
...
<p>CSS Fonts Level 4 は、色と軸と合成を CSS 側で触れるようにする。ブラウザが知らない形式は、そもそも取りにいかせない、もここに入る。</p>
...
<p>02のスライスの先が IFT です。最初は、そのページに必要なグリフだけを送る。あとから見た字は、差分で足す。Google Fonts の静的スライスより、ページごとの無駄が少ない。</p>
...
<p class="muted">常用漢字を最初から全部送る必要がなくなる。02で見た切り方の、次の段階です。</p>
